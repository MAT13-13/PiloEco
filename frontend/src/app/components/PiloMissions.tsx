...
export default function PiloMissions({ missions }: Props) {
  ...
  if (sortedMissions.length === 0) {
    return (
      <section className="mt-12 rounded-[2rem] border border-green-500/20 bg-green-500/10 p-8 text-center">
        ...
        <p className="mt-4 text-green-300">Continue comme ça 💚</p>
      </section>
    );
  }
        ...
        {sortedMissions.map((mission) => (
          ...
          >
            ...
            <Link
              ...
              className="shrink-0 rounded-full bg-green-500 px-4 py-2 text-sm font-bold text-slate-950 transition hover:scale-105 hover:bg-green-400"
            >
              {mission.status === "Terminée" ? "Voir" : "Commencer"}
            </Link>
          </div>
        ))}
      </div>
