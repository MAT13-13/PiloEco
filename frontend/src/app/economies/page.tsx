...
export default function Page() {
  ...
  const progression = Math.min(
    100,
    Math.round(
      (economise / objectif) * 100
    )
  );

  const restePotentiel =
    Math.max(
      0,
      totalPotentiel - economise
    );
      ...
      <div className="mx-auto max-w-6xl space-y-8">
        <div>
          ...
          <p className="mt-4 max-w-2xl text-slate-300">
            Visualise les économies détectées
            par Pilo, celles déjà réalisées et
            les prochaines opportunités à
            activer.
          </p>
        </div>
        ...
        <section className="rounded-3xl border border-green-500/20 bg-gradient-to-br from-slate-900 to-green-950/40 p-6">
          ...
          <p className="mt-4 text-slate-300">
            Tu pourrais encore économiser
            environ{" "}
            <span className="font-black text-green-400">
              {restePotentiel.toLocaleString(
                "fr-FR"
              )}{" "}
              €/an
            </span>
            . Je continue de surveiller tes
            contrats pour te prévenir dès
            qu’une meilleure opportunité
            apparaît.
          </p>
        </section>
      </div>
