...
export default function DashboardPage() {
  ...
  useEffect(() => {
    document.title =
      "PiloEco — Économisez sur vos factures et réduisez vos dépenses";

    const description =
      "Réduisez vos factures d’énergie, de mobile, d’internet et d’assurances, et découvrez les aides disponibles pour vos travaux avec PiloEco.";
  ...
  useEffect(() => {
    ...
    try {
      ...
      void calculerAnalyseAutomatique(parsedValues, user.id);
    } catch (error) {
      console.error("Impossible de récupérer les valeurs Pilo :", error);
  ...
  async function calculerAnalyseAutomatique(
    ...
    setChargement(true);
    setMessage("Pilo met ton analyse à jour...");
    ...
    try {
      ...
      if (error) {
        setMessage("Analyse terminée, mais elle n'a pas pu être enregistrée.");
      } else {
        ...
        setMessage("Analyse mise à jour avec succès.");
      }
    } catch (error) {
      console.error(error);

      setMessage("Impossible de mettre l'analyse à jour.");
    }
  ...
  const premiumActif = profile?.premium === true;

  const missionsRestantes = Math.max(
    0,
    missions.length - missionsTerminees.length,
  );
  ...
  const piloSituation = (() => {
    if (missionPrioritaire && Number(missionPrioritaire.saving || 0) > 0) {
      return {
        emoji: "🎯",
    ...
    if (economieAnnuelleAffichee > 0 && missionsRestantes === 0) {
      return {
        emoji: "🎉",
    ...
    if (analyses.length === 0) {
      return {
        ...
        message: "Quelques réponses suffisent pour lancer Pilo.",
    ...
    return {
      emoji: "🐦",
      title: "Pilo veille",
      message: piloBrain.pilo.message || "Ton tableau de bord est à jour.",
            ...
            <section className="mt-3 overflow-hidden rounded-2xl border border-green-500/20 bg-gradient-to-br from-slate-900 via-slate-950 to-green-950/30 p-4">
              ...
              <div className="mt-3 grid grid-cols-3 gap-2">
                ...
                <div className="rounded-xl border border-white/10 bg-slate-950/40 px-2 py-2 text-center">
                  ...
                  <p className="mt-0.5 text-xs font-black text-green-400">
                    {scoreProgression}%
                  </p>
                </div>
              </div>
                  ...
                  <div className="min-w-0">
                    ...
                    {Number(missionPrioritaire.saving || 0) > 0 && (
                      <p className="mt-0.5 text-[11px] font-bold text-green-300">
                        Jusqu&apos;à{" "}
                        {Number(missionPrioritaire.saving || 0).toLocaleString(
                          "fr-FR",
                        )}{" "}
                        € / an
                      </p>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-0.5 text-[10px] text-slate-500">
                      Monitoring, alertes et PiloLife actifs.
                    </p>
                  </div>
                  <span className="rounded-full bg-purple-500/10 px-2 py-1 text-[9px] font-black text-purple-300">
                    ACTIF
                  </span>
                </div>
              </section>
              ...
              <div className="mt-3 grid grid-cols-3 gap-2">
                ...
                <div className="rounded-xl bg-slate-950/60 p-2.5">
                  ...
                  <p className="mt-1 text-sm font-black">
                    {potentielRestant.toLocaleString("fr-FR")} €
                  </p>
                </div>
                <div className="rounded-xl bg-slate-950/60 p-2.5">
                  <p className="text-[9px] text-slate-500">Terminées</p>
                  <p className="mt-1 text-sm font-black">
                    {missionsTerminees.length}/{missions.length}
                  </p>
                </div>
              </div>
            </section>

            <section className="mt-3 rounded-2xl border border-green-500/20 bg-green-500/5 p-3.5">
              <p className="text-[9px] font-black uppercase tracking-[0.18em] text-green-400">
                🐦 Faire des économies avec Pilo
              </p>

              <h2 className="mt-1.5 text-sm font-black text-white">
                Réduis tes factures et dépense moins chaque mois
              </h2>

              <p className="mt-1.5 text-[10px] leading-4 text-slate-400">
                Énergie, mobile, internet, assurances, crédits, mutuelle,
                logement ou travaux : Pilo t’aide à repérer les économies et les
                aides adaptées à ta situation.
              </p>
            </section>
                ...
                <div className="grid items-center gap-6 lg:grid-cols-[1fr_200px]">
                  <div>
                    ...
                    <div className="mt-5 flex max-w-2xl items-start gap-3 rounded-2xl border border-white/10 bg-slate-950/40 p-4">
                      <span className="text-xl">{piloSituation.emoji}</span>
                    ...
                    <div className="mt-5 flex flex-wrap gap-2">
                      ...
                      <span className="rounded-full border border-white/10 bg-slate-950/50 px-3 py-1.5 text-xs font-bold text-slate-300">
                        {missionsTerminees.length} missions terminées
                      </span>
                ...
                <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                  <div className="max-w-3xl">
                    ...
                    <h2 className="mt-2 text-2xl font-black text-white">
                      Fais des économies sur tes dépenses du quotidien
                    </h2>

                    <p className="mt-3 text-sm leading-6 text-slate-300">
                      Tu cherches à économiser sur tes factures et à dépenser
                      moins chaque mois ? Pilo t&apos;aide à identifier
                      simplement les économies possibles sur tes principales
                      dépenses : électricité, gaz, forfait mobile, internet,
                      assurances, crédits, mutuelle, automobile, logement et
                      services du quotidien.
                    </p>

                    <p className="mt-2 text-sm leading-6 text-slate-400">
                      Pilo t&apos;accompagne aussi pour découvrir les aides
                      disponibles pour tes travaux et ton logement, comparer des
                      solutions adaptées à tes besoins et trouver de nouvelles
                      façons de réduire tes dépenses et d&apos;améliorer ton
                      pouvoir d&apos;achat. Analyse ta situation, découvre tes
                      missions d&apos;économies et trouve les solutions qui
                      peuvent réellement alléger ton budget.
                    </p>
                  </div>
              ...
              <section className="mt-5">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">
                          Réponds à quelques questions. Pilo analyse ta
                          situation et identifie les économies et missions
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-3">
                  ...
                  >
                    ...
                    <p className="mt-4 font-black text-white">Mes missions</p>
                  ...
                  >
                    ...
                    <div className="mt-4 flex items-center gap-2">
                      <p className="font-black text-white">Monitoring</p>
                  ...
                  >
                    ...
                    <div className="mt-4 flex items-center gap-2">
                      <p className="font-black text-white">PiloLife</p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      Tes missions vont être actualisées automatiquement.
                    </p>
                  </div>
                </div>
              </section>
